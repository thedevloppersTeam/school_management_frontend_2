...
async function computeClassMean(
  classSessionId: string,
  stepId: string,
  excludeEnrollmentId: string
): Promise<number | null> {
  try {
    // 1. Fetch tous les enrollments de la classe
    const enrollmentsRes = await fetch(
      `/api/enrollments?classSessionId=${classSessionId}&status=ACTIVE`,
      { credentials: 'include' }
    )
    if (!enrollmentsRes.ok) return null
    const enrollments: any[] = await enrollmentsRes.json()

    // 2. Fetch les class subjects (une seule fois pour toute la classe)
    const classSubjects = await fetchClassSubjects(classSessionId)

    // 3. Pour chaque enrollment, calculer sa moyenne générale
    const averages: number[] = []
    for (const enr of enrollments) {
      // Skip l'élève en cours (pour ne pas le compter 2 fois dans le batch)
      if (enr.id === excludeEnrollmentId) continue

      // Fetch les notes de cet élève
      const gradesRes = await fetch(
        `/api/grades/enrollment/${enr.id}?stepId=${stepId}`,
        { credentials: 'include' }
      )
      if (!gradesRes.ok) continue
      const grades: any[] = await gradesRes.json()

      // Construire l'index et calculer moyennes rubriques
      const gradeIndex = buildGradeIndex(grades)
      const { r1, r2, r3 } = buildRubriques(classSubjects, gradeIndex)
      const moyR1 = rubriqueAvg(r1)
      const moyR2 = rubriqueAvg(r2)
      const moyR3 = rubriqueAvg(r3)

      // Appliquer BR-001 (70/25/5)
      const finalAvg = ((moyR1 ?? 0) * 0.70) + ((moyR2 ?? 0) * 0.25) + ((moyR3 ?? 0) * 0.05)
      
      // Ne compter que les élèves avec au moins une note
      if (moyR1 !== null || moyR2 !== null || moyR3 !== null) {
        averages.push(finalAvg)
      }
    }

    // 4. Retourner la moyenne de ces moyennes
    return averages.length > 0 ? avg(averages) : null
  } catch (err) {
    console.error('[computeClassMean] error:', err)
    return null
  }
}
...
export async function buildBulletinData(params: {
  ...
  // 7. Moyenne de classe (calcul réel)
  const classMean = await computeClassMean(classSessionId, stepId, enrollmentId)
  ...
  return {
    ...
    moyenneClasse: classMean !== null ? classMean.toFixed(2) : '—',

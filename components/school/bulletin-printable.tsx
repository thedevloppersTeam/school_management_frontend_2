...
const PDF_HEADER_FOUNDED_FONT = 'var(--font-lobster), "Lobster", cursive';
...
function PdfSvgTextLine({
  ...
  useLayoutEffect(() => {
    ...
    const renderBitmap = () => {
      ...
      const pixelRatio = Math.max(2, Math.min(4, window.devicePixelRatio || 1));
      ...
      context.textAlign =
        align === "start" ? "left" : align === "middle" ? "center" : "right";

      const x = align === "start" ? 0 : align === "middle" ? width / 2 : width;
...
function PdfInlineSvgText({
  ...
  return (
    <svg
      className={
        className ? `pdf-inline-svg-text ${className}` : "pdf-inline-svg-text"
      }
      width="100%"
      height="100%"
      aria-hidden="true"
...
function RubriqueTable({
  ...
  const lastNoteIndex = entries.reduce(
    (lastIndex, entry, index) => (entry.isParent ? lastIndex : index),
    -1,
  );
      ...
      <div className="rubrique-score-grid">
        ...
        {entries.map((e, i) => {
          ...
          return (
            ...
            >
              ...
              >
                <PdfInlineSvgText
                  value={formatBulletinNumber(e.note)}
                  align="middle"
                />
              </div>
        ...
        <div className="rubrique-row rubrique-total-row">
          ...
          <div className="coeff-cell total-cell">
            <span className="total-value">
              <PdfInlineSvgText value={fmtC(totals.coeff)} align="middle" />
            </span>
          </div>
        </div>
      </div>
...
function BehTable({
  items,
  right = false,
}: Readonly<{ items: ComportementItem[]; right?: boolean }>) {
...
function getPeriodDetails(period: string): {
  ...
  const details = detailsByStep[stepNumber];
  ...
  return {
    label: `Période ${details.periodNumber}`,
...
export function BulletinPrintable({
  ...
  const behaviorMetricTones = {
    absences: colorClass(parseDisplayNumber(comp.absences), 10),
    retards: colorClass(parseDisplayNumber(comp.retards), 10),
    devoirsNonRemis: colorClass(parseDisplayNumber(comp.devoirsNonRemis), 10),
    leconsNonSues: colorClass(parseDisplayNumber(comp.leconsNonSues), 10),
    uniforme: colorClass(parseDisplayNumber(comp.uniforme), 10),
    discipline: colorClass(parseDisplayNumber(comp.discipline), 10),
  };
  ...
  const moyClasse =
    data.moyenneClasse && data.moyenneClasse !== "" ? data.moyenneClasse : "—";
              ...
              <div className="rubric-column rubric-column-2">
                ...
                <div className="etape-block">
                  <MoyLine
                    ...
                    valueTone={colorClass(moyenneEtapeNumber, 10)}
                    variant="step"
                  />
                  <MoyLine
                    label="Appréciation"
                  ...
                  <MoyLine
                    label="Moyenne classe sur 10"
                    value={moyClasse}
                    valueTone={colorClass(moyClasseNumber, 10)}
          ...
          <div className="lower-section">
            ...
            <div className="behavior-metrics-grid">
              <div>
                <BehaviorMetricLine
                  ...
                  valueTone={behaviorMetricTones.absences}
                  labelAlign="start"
                />
                <BehaviorMetricLine
                  label="Nbre de retards"
                  value={behaviorMetrics.retards}
                  valueTone={behaviorMetricTones.retards}
                  labelAlign="start"
                />
              </div>
              <div>
                <BehaviorMetricLine
                  ...
                  valueTone={behaviorMetricTones.devoirsNonRemis}
                />
                <BehaviorMetricLine
                  label="Nbre de leçons non sues"
                  value={behaviorMetrics.leconsNonSues}
                  valueTone={behaviorMetricTones.leconsNonSues}
                />
              </div>
              <div>
                <BehaviorMetricLine
                  label={"Respect des prescrits de l'uniforme"}
                  value={behaviorMetrics.uniforme}
                  valueTone={behaviorMetricTones.uniforme}
                />
                <BehaviorMetricLine
                  label="Discipline"
                  value={behaviorMetrics.discipline}
                  valueTone={behaviorMetricTones.discipline}
                />
              </div>
            </div>
            ...
            <div className="observations-grid">
              <div className="observation-main">
                ...
                <div className="h-maroon2">
                  <PdfInlineSvgText value="Point(s) fort(s) :" align="start" />
                </div>
                <p className="plain">{comp.pointsForts || " "}</p>
                <div className="h-maroon2 challenge-label">
                  <PdfInlineSvgText value="Défi(s) à relever :" align="start" />
                </div>
              ...
              <div className="remark-block">
                <div className="h-maroon">
                  <PdfInlineSvgText value="Remarque" align="start" />
                </div>
            ...
            <div className="bottom-area">
              <div className="legend-col">
                <div className="legend-title" data-pdf-anchor="footer-legend">
                  <PdfInlineSvgText
                ...
                <div className="legend legend-grid">
                  ...
                  <div className="legend-item">
                    51 - 59 :{" "}
                    <b style={{ color: "var(--orange)" }}>D = Déficient</b>
                  </div>
                  ...
                  <div className="legend-item">
                    60 - 68 :{" "}
                    <b style={{ color: "var(--green)" }}>C = Assez bien</b>
                  </div>
                  <div className="legend-item">
                    &le; 50 : <b style={{ color: "var(--red)" }}>E = Échec</b>
                  </div>
                </div>
                <div className="footer-rule">
                  <div className="first">
                    Seuil de réussite pour promotion automatique en classe
                    supérieure 7.00
                  </div>
                  <div>
                    Calcul de la moyenne de l&apos;étape : 70% Rubrique 1 + 25%
                    Rubrique 2 + 5% Rubrique 3
                  </div>
                  <div>
                    Calcul de la moyenne générale : Somme des étapes / nbre
                    d&apos;Étapes
                  </div>
                </div>
                <div className="contact">
                  {etab.adresse} / Téléphone : {etab.telephone} / Courriel :{" "}
                  {etab.email}
                </div>
              </div>
              ...
              <div className="sign-col">
                <div className="signbox">
                  ...
                  <div className="signlabel">
                    <PdfInlineSvgText value="Parent" align="middle" />
                  </div>
                </div>
                <div className="signbox signbox-direction">
                  ...
                  >
                    <PdfInlineSvgText value="Direction" align="middle" />
                  </div>
                </div>
              </div>
            </div>
...
.btpl .mline-general{
  ...
  /* Padding vertical uniquement pour conserver le même alignement horizontal */
  padding:
...
.btpl .behavior-choice-cell{
  ...
  /* Ligne horizontale juste sous OUI et NON */
.btpl .behavior-choice-header{
  border-bottom:calc(.4pt / var(--template-scale)) solid #000;
}
.btpl .behavior-check{
